...
void WtoB(UINT Left32, UINT Right32, BYTE* out) {
	...
	for (i = 0; i < 8; ++i)
	{
		if (i < 4)
		{
			out[i] |= (Left32 & (mask >> (i * 8))) >> (32 - (i + 1) * 8);
		}
		else
		{
			out[i] |= (Right32 & (mask >> (i * 8))) >> (64 - (i + 1) * 8);
		}
	}
}
...
void Key_Expansion(BYTE* key, BYTE round_key[DES_ROUND][6]) {
	...
	for (i = 0; i < DES_ROUND; ++i)
	{
		//	28-bit data를 좌측으로 Circular Shfit
		c = Cir_Shift(c, i);
		d = Cir_Shift(d, i);
